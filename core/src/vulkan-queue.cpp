...
namespace VK {
    ...
    u32 VulkanQueue::AcquireNextImage() {
        VkResult res = vkWaitForFences(m_device, 1, &m_inFlightFences[m_frameIndex], VK_TRUE, UINT64_MAX);
        CHECK_VK_RESULT(res, "vkWaitForFences");

        res = vkResetFences(m_device, 1, &m_inFlightFences[m_frameIndex]);
    ...
    void VulkanQueue::SubmitSync(VkCommandBuffer CmbBuf) {
        ...
        CHECK_VK_RESULT(res, "vkWaitForFences (SubmitSync)");
