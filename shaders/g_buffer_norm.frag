...
void main()
{
    ...
    vec3 sampledNormal = texture(normalSampler, texCoord).rgb;

    if (length(sampledNormal) < 0.01) {
        sampledNormal = vec3(0.5, 0.5, 1.0);
    }
    ...
    out_Normal    = vec4(finalNormal, roughnessColor.r);
    out_Depth     = linDepth;
    out_MotionVec = motion;
    out_Metallic = vec4(metallicColor.r, geomNormal);
